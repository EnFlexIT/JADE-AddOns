...
public final class SonicMqProviderAdmin implements ProviderAdmin {
  ...
  public Queue getOrCreateQueue (JMSAddress jmsTA) throws MTPException {
    ...
    try {
      ...
      domain.connect(JmsMtpConfig.getProperty(classKey + "domain.name", 
                                              "Domain1"), 
                     "tcp://" + jmsTA.getBrokerURL(), 
                     JmsMtpConfig.getProperty(classKey + "domain.username", 
                                              "Administrator" // Username
                                              ), 
                     JmsMtpConfig.getProperty(classKey + "domain.password", 
                                              "Administrator" // Password
                                              ));

      // Load the Broker Bean
      IBrokerBean broker = domain.getBrokerBean(JmsMtpConfig.getProperty(
                                                      classKey + "domain.brokerViewName", 
                                                      "/Brokers/Broker1"));
      ...
      try {
        maxQueueSize = Integer.parseInt(JmsMtpConfig.getProperty(
                                              classKey + "queue.maxSize", 
                                              "50000"));
      } catch (Exception any) {
      ...
      try {
        queueSaveThr = Integer.parseInt(JmsMtpConfig.getProperty(
                                              classKey + "Queue.saveThreshold", 
                                              "25000"));
      } catch (Exception any) {
