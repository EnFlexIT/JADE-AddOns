...
public class FipaXMLUtil {
  ...
  public FipaXMLUtil () {
    ...
    try {
      ...
      builder = new SAXBuilder(JmsMtpConfig.getProperty(
                                   "ie.nuigalway.ecrg.jade.jmsmtp.fipaxmlutil.parser", 
                                   "org.apache.xerces.parsers.SAXParser").trim(), 
                             false);
                             
    } catch (Exception e) {
      log.error("Error Creating XML outputter" + e.toString());
      log.error("Failed to create SaxBuilder:" + e.toString());
    } 
  ...
  public String encode (Envelope env, byte[] payload) {
    ...
    if ((env.getComments() != null) && 
        (! env.getComments().trim().equalsIgnoreCase(""))) {
    ...
    if ((env.getAclRepresentation() != null) && 
        (! env.getAclRepresentation().trim().equalsIgnoreCase(""))) {
    ...
    if ((env.getPayloadEncoding() != null) && 
        (! env.getPayloadEncoding().trim().equalsIgnoreCase(""))) {
    ...
    if ((rcv != null) && 
        ((rcv.getBy() != null) || (rcv.getFrom() != null) || 
          (rcv.getDate() != null) || (rcv.getVia() != null) || 
          (rcv.getId() != null))) {
    ...
    try {
      ...
      log.error("Error outputting XML" + e.toString());
    }

    if (log.isDebugEnabled()) {
      log.debug("Message being sent: " + res);
    }
  ...
  private AID decodeAid (Element elAid) {
    ...
    if (log.isDebugEnabled()) {
      log.debug("Creating AID for: " + aid.getName());
    }
  ...
  public Envelope decode (String msg, StringBuffer payload) {
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        log.debug("Message received: " + outputter.outputString(doc));
      }
    ...
    // NEED to lookingot the mulit pram stuff for non jade platforms and the like!!!!!!
    while (itParams.hasNext()) {
      ...
      if (elParams.getChildText("acl-representation") != null) {
        envelope.setAclRepresentation(elParams.getChildText(
                                            "acl-representation"));
      }

      // Payload Lenght
      if (elParams.getChildText("payload-length") != null) {
        envelope.setPayloadLength(new Long(elParams.getChildText(
                                                 "payload-length")));
      }
      ...
      if (elParams.getChild("received") != null) {
        ...
        ro.setBy(((Element) received.getChild("received-by")).getAttribute(
                       "value").toString());
        ro.setFrom(((Element) received.getChild("received-from")).getAttribute(
                         "value").toString());
        ro.setDate(this.getDateFromString(((Element) received.getChild(
                                                 "received-date")).getAttribute(
                                                "value").toString()));
        ro.setId(((Element) received.getChild("received-id")).getAttribute(
                       "value").toString());
        ro.setVia(((Element) received.getChild("received-via")).getAttribute(
                        "value").toString());
