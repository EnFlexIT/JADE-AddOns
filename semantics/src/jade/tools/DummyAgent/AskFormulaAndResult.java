...
//    }
    ...
    public AskFormulaAndResult(String textToBePrompt, String title, String type, String oldValue, DummyAgent agt, DefaultListModel model) {
        ...
        answerPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);    
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    if (nomProjet.getText() != null && !nomProjet.getText().trim().equals("")) {
                        result = (IdentifyingExpression)SL.fromTerm(nomProjet.getText());
                    } else {
                        result = null;
                    }
                    if (result != null) {
                                ...
                                if ( !line.equals("") && !line.startsWith("//")) {
                                    try {
                                        if (line != null && !line.trim().equals("")) {
                                            ...
                                            Formula f = SL.fromFormula(line);
                                            ListOfMatchResults result = myAgent.getSemanticCapabilities().getMyKBase().query(f);
                                            if (result != null) {
                            ...
                            while ( (line = reader.readLine()) != null ) {
                                ...
                                if ( !line.equals("") && !line.startsWith("//")) {
                                    try {
                                        IdentifyingExpression result;
                                        if (line != null && !line.trim().equals("")) {
                                            result = (IdentifyingExpression)SL.fromTerm(line);
                                        } else {
                                            result = null;
                                        }
                                        if (result != null) {
    ...
    public class OkAct implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            try {
                answerArea.setText("");
                if (nomProjet.getText() != null && !nomProjet.getText().trim().equals("")) {
                    Formula f = SL.fromFormula(nomProjet.getText());
                    ListOfMatchResults result = myAgent.getSemanticCapabilities().getMyKBase().query(f);
                    if (result != null) {
