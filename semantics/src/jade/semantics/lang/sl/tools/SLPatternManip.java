...
public class SLPatternManip {
	...
	public static class LoopingInstanciationException extends Exception {
		public LoopingInstanciationException(Node meta, Node value) {
	...
	public static void set(Node expression, String name, Node value)
		...
		if (expression.find(MetaActionExpressionReferenceNode.class, "lx_name",
				name, metaReferences, true)
				|| expression.find(MetaContentExpressionReferenceNode.class,
						"lx_name", name, metaReferences, true)
				|| expression.find(MetaFormulaReferenceNode.class, "lx_name",
						name, metaReferences, true)
				|| expression.find(MetaSymbolReferenceNode.class, "lx_name",
						name, metaReferences, true)
				|| expression.find(MetaTermReferenceNode.class, "lx_name",
						name, metaReferences, true)
				|| expression.find(MetaVariableReferenceNode.class, "lx_name",
						name, metaReferences, true)) {
	...
	public static Node instantiate(Node expression) {
		try {
			Node result = expression.getClone();
			metaVariables2Value(result);
			return result;
		} catch (LoopingInstanciationException lie) {
			System.err.println("Exception occurs when trying to instanciate "
					+ expression);
			lie.printStackTrace();
			return null;
		}
	...
	public static Node instantiate(Node expression, String varname1,
			...
			throws WrongTypeException {
		set(expression, varname1, value1);
		return instantiate(expression, varname2, value2);
	}
	...
	public static Node instantiate(Node expression, String varname1,
		...
		set(expression, varname1, value1);
		return instantiate(expression, varname2, value2, varname3, value3);
	}
	...
	public static class Matcher extends VisitorBase {
		...
		// -----------------------------------------------
		{
			for (int i = 0; i < _metaReferences.size(); i++) {
				...
				ListOfNodes metaReferences = new ListOfNodes();
				if (_expression1.find(var.getClass(), "lx_name", name,
						metaReferences, true)
						|| _expression2.find(var.getClass(), "lx_name", name,
								metaReferences, true)) {
		...
		// -----------------------------------------------
		{
			for (int i = 0; i < _metaReferences.size(); i++) {
				Node var = _metaReferences.get(i);
				try {
					set(_expression1, var.toString(), null);
					set(_expression2, var.toString(), null);
		...
		public void visitMetaActionExpressionReferenceNode(
				MetaActionExpressionReferenceNode node) {
			if (_expression2 instanceof ActionExpression) {
				doPatternMatchingOnMetaReference(node, _expression2);
			} else {
				_match = false;
			}
		...
		public void visitParameterNode(ParameterNode node) {
			if (_expression2 instanceof ParameterNode) {
				_match = (node.lx_name().equals(
						...
						node.as_value(), ((ParameterNode) _expression2)
								.as_value()));
			} else {
				_match = false;
			}
	...
	// -----------------------------------------------
	{
		try {
			if (metaReference instanceof MetaFormulaReferenceNode) {
			...
			} else if (metaReference instanceof MetaActionExpressionReferenceNode) {
				MetaActionExpressionReferenceNode mRef = (MetaActionExpressionReferenceNode)metaReference;
				if (value instanceof ActionExpression) {
					mRef.sm_value((ActionExpression) value);
				} else {
					mRef.sm_value(((ActionContentExpressionNode) value).as_action_expression());
				}
				...
				if (value instanceof ContentExpression) {
	...
	// -----------------------------------------------
	private static void metaVariables2Value(Node node)
			throws LoopingInstanciationException
	// -----------------------------------------------
	{
		Node[] children = node.children();
		...
		for (int i = 0; i < children.length; i++) {
			...
			while ((childNode instanceof MetaFormulaReferenceNode
					...
					|| childNode instanceof MetaActionExpressionReferenceNode || childNode instanceof MetaContentExpressionReferenceNode)
					&& ((valueNode = getMetaReferenceValue(childNode)) != null)) {
				if (childNode == valueNode) {
					throw new LoopingInstanciationException(childNode,
							valueNode);
				}
				childNode = valueNode;
			}
			// childNode is not a meta-reference or its value is null
			metaVariables2Value(childNode);
