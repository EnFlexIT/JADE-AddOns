	...
	public class SLEqualizer {
		...
		public boolean doEquals(Node expression1, Node expression2) {
			...
			if ( equal ) {
				...
				if ( equal ) {
					if ( expression1 instanceof MetaActionExpressionReferenceNode ) {
						equal = ((MetaActionExpressionReferenceNode)expression1).lx_name()
						.equals(((MetaActionExpressionReferenceNode)expression2).lx_name());
					}
						...
						.equals(((StringConstantNode)expression2).lx_value());
					}
					else if ( expression1 instanceof WordConstantNode ) {
						equal = ((WordConstantNode)expression1).lx_value()
						.equals(((WordConstantNode)expression2).lx_value());
					}
					else if ( expression1 instanceof ByteConstantNode ) {
						equal = ((ByteConstantNode)expression1).lx_value()
						.equals(((ByteConstantNode)expression2).lx_value());
					}
