...
public class MatchResult extends ListOfNodes {
	...
	/**
	 * Build a match result by cloning the one given as an argument
	 * @param match
	 */
	public MatchResult(MatchResult match) {
		super(match.getClone().children());
	}
	...
	public boolean equals(Object object)
	{
		...
		if ( isEqual && size() != 0 ) {
			...
			do {
				...
				isEqual = l2.find(n1.getClass(), "lx_name", SL.getMetaReferenceName(n1), lo, true);
				if ( isEqual ){
					Node n2 = lo.getFirst();
	...
	public MatchResult intersect(MatchResult other)
	{
		...
		for (int i = 0; i < size(); i++) {
			Node m = get(i);
			ListOfNodes lo = new ListOfNodes();
			if ( other.find(m.getClass(), "lx_name", SL.getMetaReferenceName(m), lo, true) ) {
    ...
    public void copyValueOf(Node n) {
        if (n instanceof MatchResult) {
			...
			for (int i = 0; i < size(); i++) {
				Node meta = get(i);
				String name = SL.getMetaReferenceName(meta);
				Node value = SL.getMetaReferenceValue(meta);
				ListOfNodes linked = new ListOfNodes();
				if (value instanceof MetaContentExpressionReferenceNode ||
					...
					if ( find(META_REFERENCE_CLASSES, "lx_name", valuename, othermeta, false) ) {
						try {
    ...
    public void completeClosure() throws WrongTypeException {
        ...
        for (int i=0; i<metaReferences.size(); i++) {
        	...
        	if (SL.getMetaReferenceValue(metaToAssign) == null) {
        		...
        		find(META_REFERENCE_CLASSES, "lx_name", SL.getMetaReferenceName(metaToAssign), result, false);
	 ...
	 */
	protected ListOfNodes restore(int size) {
		ListOfNodes result = new ListOfNodes();
