...
public class MatchResult extends ListOfNodes {
	...
	public Formula getFormula(String name) throws WrongTypeException {
		...
		Formula result = null;
		if (find(MetaFormulaReferenceNode.class, "lx_name", name, metaReferences, true)) {
	...
	public Term getTerm(String name) throws WrongTypeException {
		ListOfNodes metaReferences = new ListOfNodes();
		Term result = null;
		if (find(MetaTermReferenceNode.class, "lx_name", name, metaReferences, true)) {
			result = ((MetaTermReferenceNode) metaReferences.getFirst())
			.sm_value();
		} else if (find(MetaVariableReferenceNode.class, "lx_name", name, metaReferences, true)) {
			result = ((MetaVariableReferenceNode) metaReferences.getFirst())
			.sm_value();
		}
		else {
			result = getActionExpression(name);
		}
		return result;
	}
	...
	public Variable getVariable(String name) throws WrongTypeException {
		...
		if (find(MetaVariableReferenceNode.class, "lx_name", name, metaReferences, true)) {
	...
	public Symbol getSymbol(String name) throws WrongTypeException {
		ListOfNodes metaReferences = new ListOfNodes();
		Symbol result = null;
		if (find(MetaSymbolReferenceNode.class, "lx_name", name, metaReferences, true)) {
	...
	/**
	 * Return the action expression assigned to the named meta variable.
	 * 
	 * @param name
	 *            the name of the meta variable to get its value
	 * @return the action expression which is the value of the meta variable
	 *         named with the given <b><code>name</code></b>, or <b><code>null</code></b>
	 *         if no action expression is assigned to this meta variable,
	 *         meaning the last matching operation has failed, or no
	 *         matching operation has been performed on this pattern.
	 * @exception WrongTypeException
	 *                Thrown if the value of the variable is not a action
	 *                expression.
	 */
	public ActionExpression getActionExpression(String name)
			throws WrongTypeException {
		ListOfNodes metaReferences = new ListOfNodes();
		ActionExpression result = null;
		if (find(MetaActionExpressionReferenceNode.class, "lx_name", name, metaReferences, true)) {
			result = ((MetaActionExpressionReferenceNode) metaReferences
					.getFirst()).sm_value();
		}
		return result;
	}
	...
	public ContentExpression getContentExpression(String name)
		...
		if (find(MetaContentExpressionReferenceNode.class, "lx_name", name, metaReferences, true)) {
