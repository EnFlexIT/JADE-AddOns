...
public abstract class CommunicativeActionImpl extends SemanticActionImpl  implements CommunicativeAction, CommunicativeActionProto {
	 ...
	 */
	public CommunicativeActionImpl(SemanticActionTable table, int surfacePerformative,
		...
		try {
			this.myActionExpressionPattern = (Term)SLPatternManip.instantiate(COMMUNICATIVE_ACTION_PATTERN,
                    "performative", new SymbolNode(ACLMessage.getPerformative(surfacePerformative)));
     ...
     * @throws SemanticInterpretationException if any exception occurs
     */
    static public SemanticAction newAction(ActionExpression actionExpression, CommunicativeActionProto act) throws SemanticInterpretationException {
        MatchResult matchResult = SLPatternManip.match(act.getMyActionExpressionPattern(), actionExpression);
        if (matchResult != null) {
            ...
            try {
                ...
                result.setSurfaceContent(act.parseContent(((StringConstantNode)matchResult.getTerm("content")).lx_value(), result.getLanguage()));
            }
    ...
    static public SemanticAction newAction(ACLMessage aclMessage, CommunicativeActionProto act) throws SemanticInterpretationException {
        if (aclMessage.getPerformative() == act.getSurfacePerformative()) {
            ...
            result.setSurfaceContent(act.parseContent(aclMessage.getContent(), act.getLanguage()));
    ...
    static public SemanticAction newAction(Content content, CommunicativeAction body, CommunicativeActionProto act) throws SemanticInterpretationException {
        CommunicativeActionProto result = act.createInstance();
        result.setAuthor(body.getAuthor());
    ...
    static public SemanticAction newAction(Formula rationalEffect, ACLMessage inReplyTo, CommunicativeActionProto act) {
        MatchResult matchResult = SLPatternManip.match(act.getRationalEffectRecognitionPattern(), rationalEffect);
        if (matchResult != null) {
            CommunicativeActionProto result = act.createInstance();
            result.setAuthor(act.getSemanticActionTable().getSemanticCapabilities().getAgentName());
            if (inReplyTo != null) {
                result.setLanguage(inReplyTo.getLanguage());
                result.setOntology(inReplyTo.getOntology());
                result.setProtocol(inReplyTo.getProtocol());
    ...
    static public SemanticAction buildAction(CommunicativeActionProto act) throws SemanticInterpretationException {
        if (act.getAuthor() != null && act.getReceiverList() != null && act.getReceiverList().size() > 0) {
            ...
            throw new SemanticInterpretationException("missing-parametter", new StringConstantNode("sender or receiver"));
        }
    ...
    static public ContentNode parseContent(String content, String language, CommunicativeActionProto act) throws SemanticInterpretationException { 
        try {
            ContentParser contentParser = act.getSemanticActionTable().getSemanticCapabilities().getContentParser(language);
            if (contentParser == null) throw new SemanticInterpretationException("unknown-language", new StringConstantNode(language));
            return (ContentNode)contentParser.parseContent(content);
        } 
        catch (ParseContentException pce) {
            throw new SemanticInterpretationException("cannot-parse-content", new StringConstantNode(content));
        }
    ...
    public Behaviour computeBehaviour() {
        return new PrimitiveBehaviour(this);
    }
    ...
    public ActionExpression toActionExpression(boolean surface) throws SemanticInterpretationException {
        return toActionExpression(surface, this);
    }
    
    /**
     * Returns the action expression representation of this action.
     * @param surface true if the surface content should be used, false if not
     * @param act the kinf of action to be create
     * @return an action expression representing the semantic action
     * @throws SemanticInterpretationException if any exception occurs
     */
    static public ActionExpression toActionExpression(boolean surface, CommunicativeActionProto act) throws SemanticInterpretationException {
        ActionExpression result = (ActionExpression)COMMUNICATIVE_ACTION_PATTERN.getClone();
        try {
            SLPatternManip.set(result, "performative", (surface ?
                    new SymbolNode(ACLMessage.getPerformative(act.getSurfacePerformative()))
                    : new SymbolNode(ACLMessage.getPerformative(act.getPerformative()))));
            SLPatternManip.set(result, "sender", act.getAuthor());
            SLPatternManip.set(result, "receiver", new TermSetNode(act.getReceiverList()));
            SLPatternManip.set(result, "content", new StringConstantNode(
                    act.getSemanticActionTable().getSemanticCapabilities().getContentParser(act.getLanguage()).unparseContent(
                            surface ? act.getSurfaceContent() : act.getContent())));
            if (act.getLanguage() != null) SLPatternManip.set(result, "language", new WordConstantNode(act.getLanguage()));
            if (act.getOntology() != null) SLPatternManip.set(result, "ontology", new WordConstantNode(act.getOntology()));
            if (act.getEncoding() != null) SLPatternManip.set(result, "encoding", new WordConstantNode(act.getEncoding()));
            if (act.getConversationId() != null) SLPatternManip.set(result, "conversation-id", new WordConstantNode(act.getConversationId()));
            if (act.getProtocol() != null) SLPatternManip.set(result, "protocol", new WordConstantNode(act.getProtocol()));
            if (act.getReplyWith() != null) SLPatternManip.set(result, "reply-with", new WordConstantNode(act.getReplyWith()));
            if (act.getInReplyTo() != null) SLPatternManip.set(result, "in-reply-to", new WordConstantNode(act.getInReplyTo()));
            if (act.getReplyBy() != null) SLPatternManip.set(result, "reply-by", new DateTimeConstantNode(act.getReplyBy()));
            if ((act.getReplyToList() != null) && (act.getReplyToList().size() > 0))
                    SLPatternManip.set(result, "reply-to", new TermSetNode(act.getReplyToList()));
    ...
    static public ACLMessage toAclMessage(CommunicativeActionProto act) throws UnparseContentException {
        ...
        result.setSender(Tools.term2AID(act.getAuthor()));
        for (int i = 0 ; i < act.getReceiverList().size() ; i++) {
            result.addReceiver(Tools.term2AID(act.getReceiverList().element(i)));
        }
        result.setContent(act.getSemanticActionTable().getSemanticCapabilities().getContentParser(act.getLanguage()).unparseContent(act.getSurfaceContent()));
        result.setLanguage(act.getLanguage());
    ...
    public void setConversationId(WordConstantNode conversationId) {
        this.conversationId = (conversationId == null ? null : conversationId.lx_value());
    }
    /**
    ...
    public void setEncoding(WordConstantNode encoding) {
        this.encoding = (encoding == null ? null : encoding.lx_value());
    }
    /**
    ...
    public void setInReplyTo(WordConstantNode inReplyTo) {
        this.inReplyTo = (inReplyTo == null ? null : inReplyTo.lx_value());
    }
    /**
    ...
    public void setLanguage(WordConstantNode language) {
        this.language = (language == null ? null : language.lx_value());
    }
    /**
    ...
    public void setOntology(WordConstantNode ontology) {
        this.ontology = (ontology == null ? null : ontology.lx_value());
    }
    /**
    ...
    public void setProtocol(WordConstantNode protocol) {
        this.protocol = (protocol == null ? null : protocol.lx_value());
    }
    /**
    ...
    public void setReplyWith(WordConstantNode replyWith) {
        this.replyWith = (replyWith == null ? null : replyWith.lx_value());
    }
