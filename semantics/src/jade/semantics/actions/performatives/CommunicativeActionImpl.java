...
public abstract class CommunicativeActionImpl extends SemanticActionImpl  implements CommunicativeAction, CommunicativeActionProto {
     ...
     * @throws SemanticInterpretationException if any exception occurs
     */
    static public ActionExpression toActionExpression(CommunicativeActionProto act, boolean surface, Term sender, ListOfTerm receivers, Content content) throws SemanticInterpretationException {
        ...
        try {
            ...
            SL.set(result, "sender", sender == null ? act.getAuthor() : sender);
            ...
            if (act.getLanguage() != null) SL.set(result, "language", new WordConstantNode(act.getLanguage()));
            if (act.getOntology() != null) SL.set(result, "ontology", new WordConstantNode(act.getOntology()));
            if (act.getEncoding() != null) SL.set(result, "encoding", new WordConstantNode(act.getEncoding()));
            if (act.getConversationId() != null) SL.set(result, "conversation-id", new WordConstantNode(act.getConversationId()));
            if (act.getProtocol() != null) SL.set(result, "protocol", new WordConstantNode(act.getProtocol()));
            if (act.getReplyWith() != null) SL.set(result, "reply-with", new WordConstantNode(act.getReplyWith()));
            if (act.getInReplyTo() != null) SL.set(result, "in-reply-to", new WordConstantNode(act.getInReplyTo()));
            if (act.getReplyBy() != null) SL.set(result, "reply-by", new DateTimeConstantNode(act.getReplyBy()));
            if ((act.getReplyToList() != null) && (act.getReplyToList().size() > 0))
                    SL.set(result, "reply-to", new TermSetNode(act.getReplyToList()));
     ...
     * @throws SemanticInterpretationException if any exception occurs
     */
    static public ActionExpression toActionExpression(CommunicativeActionProto act, boolean surface) throws SemanticInterpretationException {
    ...
    static public ACLMessage toAclMessage(CommunicativeActionProto act) throws UnparseContentException {
        ...
        result.setContent(act.getSemanticCapabilities().getContentParser(act.getLanguage()).unparseContent(act.getSurfaceContent()));
