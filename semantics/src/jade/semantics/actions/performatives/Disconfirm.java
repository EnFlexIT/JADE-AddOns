 ...
 * receiver is known to believe, or believe it likely tath, the proposition is 
 * true.<br>
 ...
 * is false,
 * <li> the sender whishes the receiver to believe the nagated proposition also.
 * </ul> 
 * @author Vincent Pautret
 * @version Date: 2004/11/30 17:00:00  Revision: 1.0 
 */
public class Disconfirm extends CommunicativeAction {
	...
	/**
	 * Pattern used to recognize a disconfirm action
	 */
    ActionExpression disconfirmPattern;	
    
	/**
	 * Pattern used to recognize the rational effect of this kinf of action
	 */
	Formula rationalEffectRecognitionPattern;
    ...
    public Disconfirm(SemanticActionTable table) {
        super(table);
        disconfirmPattern = (ActionExpression) SLPatternManip.fromTerm("(action ??sender (DISCONFIRM :sender ??sender :receiver ??receiver :content ??content))");
        rationalEffectRecognitionPattern = SLPatternManip.fromFormula("(B ??receiver ??content)");
        feasibilityPreconditionPattern = SLPatternManip.fromFormula("(and (B ??sender (not ??content)) (B ??sender (or (B ??receiver ??content) (U ??receiver ??content))))");
        rationalEffectPattern = SLPatternManip.fromFormula("(B ??receiver (not ??content))");
        postConditionPattern = SLPatternManip.fromFormula("(B ??sender (B ??receiver (not ??content)))");
		setACLMessageCode(ACLMessage.DISCONFIRM);
    } // End of constructor Disconfirm/1
    ...
    public SemanticAction newAction(ActionExpression actionExpression) {
    	Disconfirm result = new Disconfirm(table);
		MatchResult matchResult = SLPatternManip.match(disconfirmPattern, actionExpression);
        if (matchResult != null) {
        	try {
    ...
    public SemanticAction newAction(Formula rationalEffect, Term agentName) {
		...
		try {
			MatchResult matchResult = SLPatternManip.match(rationalEffectRecognitionPattern, rationalEffect);
			if (matchResult != null) {
				result.setReceiverList(new ListOfTerm(new Term[] {matchResult.getTerm("??receiver")}));
				result.setMyContent(new NotNode(matchResult.getFormula("??content")));
