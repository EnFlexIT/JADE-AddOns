...
public class InformRef extends CommunicativeActionImpl {
    ...
    public InformRef(SemanticCapabilities capabilities,
        ...
        super(capabilities,
			  ...
			  (rationalEffectRecognition == null ? SL.fromFormula("(exists ??var (B ??receiver ??formula))") : rationalEffectRecognition),
			  SL.fromFormula("(exists ?y (B ??receiver (= ??ire ?y)))"),
			  SL.fromFormula("(and (exists ?y (B ??sender (= ??ire ?y))) (not (B ??sender (or (exists ?y (B ??receiver (= ??ire ?y))) (exists ?y (U ??receiver (= ??ire ?y))) ))))"),
			  SL.fromFormula("(exists ?y (B ??sender (= ??ire ?y)))"),
			  SL.fromFormula("(B ??sender (exists ?y (B ??receiver (= ??ire ?y))))"));
