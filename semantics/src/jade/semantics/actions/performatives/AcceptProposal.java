 ...
 * @version 
 */
public class AcceptProposal extends CommunicativeAction {

    /**
     * Pattern used to recognize an accept proposal action
     */
    Term acceptProposalMainPattern;
    ...
    public AcceptProposal(SemanticActionTable table) {
        super(table);
        acceptProposalMainPattern = SLPatternManip.fromTerm("(action ??sender (ACCEPT-PROPOSAL :sender ??sender :receiver ??receiver :content ??content))");
        ...
        setACLMessageCode(ACLMessage.ACCEPT_PROPOSAL);
    } // End of constructor AcceptProposal/1
    ...
    public SemanticAction newAction(ActionExpression actionExpression) {
        try {
            MatchResult matchResult = SLPatternManip.match(acceptProposalMainPattern, actionExpression);
            if (matchResult != null) {
