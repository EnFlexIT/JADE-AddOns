 ...
 *	
 *	class MainSemanticCapabilities extends SemanticCapabilities {
 *		protected KBase setupKbase() {
 *			FilterKBase kb = (FilterKBase) super.setupKbase();		
 *			storage = new FunctionalTermTableImpl(Main.this.getAgent());						
 *			kb.addKBQueryFilter(storage.getKBQueryFilter(this));
 *			return kb;
 *		}
 *	}
 *	
...
public interface FunctionalTermTable {
	...
	/**
	 * Retrieve a query filter to be added to a FilterKBase
	 * @param sc the semantic capabilities of the agent
	 * @return the query filter to be added to a FilterKBase
	 */
