...
public class IREFilter extends KBQueryFilter {
     ...
     */
    public IREFilter() {
        pattern = SLPatternManip.fromFormula("(B ??agent (= ??ire ??term))");
        notPattern = SLPatternManip.fromFormula("(B ??agent (not (= ??ire ??term)))");
    ...
    public QueryResult apply(Formula formula, Term agent) {
        ...
        try {
            ...
            if (applyResult != null) {
                ...
                applyResult = SLPatternManip.match(notPattern,formula);
                if (applyResult != null) {
                    if (applyResult.getTerm("ire") instanceof IdentifyingExpression) {
                        ire = (IdentifyingExpression)applyResult.getTerm("ire");
                        term = applyResult.getTerm("term");
        ...
        if (listOfMatchResults == null && myKBase.isClosed(formulaPattern, listOfMatchResults)) {
                        ...
                        for (int i=0; i<listOfNodes.size(); i++) {
                            if (termPattern instanceof MetaTermReferenceNode) {
                                ...
                                if (((MatchResult)listOfMatchResults.get(j)).getTerm(((VariableNode)listOfNodes.get(i)).lx_name()) == null) {
                                    termResult = (Term)SLPatternManip.instantiate(termResult, 
            ...
            if (term instanceof MetaTermReferenceNode) {
                ...
                if (ire instanceof IotaNode) {
                    match = SLPatternManip.match(term, result.get(0));
                } else if (ire instanceof AnyNode) {
                    match = SLPatternManip.match(term, result.get(0));
                } else {
                    match = SLPatternManip.match(term, new TermSetNode(result));   
                }
                if (match != null) {
    ...
    private void getPattern() {
        ...
        if (ire.as_term().childrenOfKind(VariableNode.class, listOfNodes)) {
            termPattern = (Term)SLPatternManip.toPattern(ire.as_term(), (VariableNode)listOfNodes.get(0), ((VariableNode)listOfNodes.get(0)).lx_name());
            formulaPattern = (Formula)SLPatternManip.toPattern(ire.as_formula(), (VariableNode)listOfNodes.get(0), ((VariableNode)listOfNodes.get(0)).lx_name());
            for (int i = 1; i < listOfNodes.size(); i++) {
                formulaPattern = (Formula)SLPatternManip.toPattern(formulaPattern, (VariableNode)listOfNodes.get(i), ((VariableNode)listOfNodes.get(i)).lx_name());
                termPattern = (Term)SLPatternManip.toPattern(termPattern, (VariableNode)listOfNodes.get(i), ((VariableNode)listOfNodes.get(i)).lx_name());
            }
        } else {
