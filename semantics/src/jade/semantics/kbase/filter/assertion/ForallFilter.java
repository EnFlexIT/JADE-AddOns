...
public class ForallFilter extends KBAssertFilter {
     ...
     */
    public ForallFilter() {
        pattern = SLPatternManip.fromFormula("(B ??agt (forall ??var ??phi))");
        pattern2 = SLPatternManip.fromFormula("(= (all ??var (not ??phi)) (set))");
    ...
    public final Formula beforeAssert(Formula formula) {
        ...
        try {
            ...
            if (applyResult != null) {
                ...
                myKBase.assertFormula((Formula)SLPatternManip.instantiate(pattern2, "??var", applyResult.getVariable("var"),
