...
public abstract class NestedBeliefFilters extends FiltersDefinition {
	...
	private Formula bPattern = SL.fromFormula("(B ??myself ??phi)");
	private Formula notBPattern = SL.fromFormula("(not (B ??myself ??phi))");
			...
			public QueryResult apply(Formula formula, Term agent) {
				...
				if (applyResult!=null) {
					...
					if (nestedBelief != null && handleNestedBeliefFrom(beliefAgent.sm_value())) {
						...
						if (nestedKBase != null) {
							result.setFilterApplied(true);
							result.setResult(nestedKBase.query(nestedFormula));
						}
					}
				}
				return result;
