...
public class FunctionalTermTableImpl implements FunctionalTermTable {
	...
	//                 PRIVATE FIELDS
	//---------------------------------------------------------------
	SemanticCapabilities myCapabilities;
	HashMap terms;
	long nlid;
	...
	/**
	 *  Looks for terms matching the classname, the required fields and the local id, 
	 ...
	 *  a list containing all the requiered fields but not the lid.
	 *  
			...
			for (int i=0; i<fts.length; i++) {
				if ( ((FT)fts[i]).getClassName().equals(classname) ) {
					...
					if ( vals.size() == fields.length ) {
						...
						result.add(new WordConstantNode(((FT)fts[i]).getID()));
	...
	class FT {
		...
		String getID() {
			return myCapabilities.getAgent().getName()+":"+getLID();
		}
		...
		{
			FunctionalTermParamNode term = new FunctionalTermParamNode(new SymbolNode(classname),
					                                                   new ListOfParameter());
	...
	/**
	 * Constructor
	 * @param sc the semantic capabilities of the agent holding the table
	 */
	public FunctionalTermTableImpl(SemanticCapabilities sc) {
		this.myCapabilities = sc;
		this.nlid = 0;
	...
	public String add(FunctionalTermParamNode term) {
		...
		if ( !terms.containsValue(ft) ) {
			...
			myCapabilities.getMyKBase().updateObservers(null);
		}
		return ft.getLID();
	}
	...
	public void load(Reader r) throws IOException {
		...
		String line = null;
		String myName = myCapabilities.getAgent().getLocalName();

		while (br.ready()) {
			line = br.readLine().trim();
			if ( line.startsWith("(") ) {
				// Functional term without a name
				term = (FunctionalTermParamNode)SL.fromTerm(complete(line.trim(), named_terms));
				term = (FunctionalTermParamNode)term.instantiate("myself", new StringConstantNode(myName));
				add(term);
				...
				if ( split.length == 2 ) {
					term = (FunctionalTermParamNode)SL.fromTerm(complete(split[1].trim(), named_terms));
					term = (FunctionalTermParamNode)term.instantiate("myself", new StringConstantNode(myName));
					String lid = add(term);		
	 ...
	 */
	public KBQueryFilter getKBQueryFilter(SemanticCapabilities sc) {
			...
			public QueryResult apply(Formula formula, Term agent) {
				...
				try {
					if ( (formula instanceof BelieveNode) && 
						 ((BelieveNode)formula).as_agent().equals(myCapabilities.getAgentName()) &&
						...
						String[] symbol = symbolNode.lx_value().split(":");
						...
						if ( symbol.length == 2 ) {
							...
							if ( (nbvals >= 1) && (terms[0] != null) && (nbvals == fields.length) ) {
									...
									if ( results != null ) {
										for ( int i=0; i<results.size(); i+=(nbvals+1) ) {
											...
											for ( int k=0; k<nbvals; k++) {
												...
												else {matchres = matchres.join(matchval[k]);}
											}
								...
								if ( list.size() != 0 ) {queryResult.setResult(list);}
							}
						}
					}
				}
