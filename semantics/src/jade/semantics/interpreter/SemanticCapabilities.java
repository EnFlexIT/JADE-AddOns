...
import jade.semantics.kbase.DefaultFilterKBaseLoader;
...
/**
 * This class makes it possible to turn a JADE agent into a semantic one, by
 * installing all components of the JSA framework on a givent agent.
 * 
 * It is intended to be extended for developing a specific JSA-based semantic
 ...
 * </ul>
 * 
 * The following code excerpt shows a typical skeleton of an application-specific
 * {@link SemanticCapabilities} subclass:
 * 
 * <pre>
...
public class SemanticCapabilities {
   ...
   /*********************************************************************/
   /**
    * Creates a new {@link SemanticCapabilities} instance.
   ...
   public void sendCommunicativeAction(CommunicativeAction action) {
	   myAgent.send(getMyACLMessageProducer().produce(action));
   } 
    ...
    *              object on
    */
   protected void install(Agent agent) {
       //
       ...
       setupStandardCustomization(); /* This method is deprecated */
...
protected SemanticActionTable setupSemanticActions() {
    ...
    */
   protected SemanticActionTable setupSemanticActions() {
       SemanticActionTable table = new SemanticActionTableImpl(this);
       new DefaultSemanticActionLoader().load(table);
	   return table;
   } 
   
   /**
    * Sets up the semantic agent's SIP table. The SIPs (Semantic Interpretation
    * Principles) are the basic rules that specify how to interprat the events
...
protected SemanticInterpretationPrincipleTable setupSemanticInterpretationPrinciples() {
    ...
    */
   protected SemanticInterpretationPrincipleTable setupSemanticInterpretationPrinciples() {
	   SemanticInterpretationPrincipleTable table = new SemanticInterpretationPrincipleTableImpl();
	   new DefaultSemanticInterpretationPrincipleLoader().load(table, this);
	   return table;
   } 
...
protected KBase setupKbase() {
    ...
    */
   protected KBase setupKbase() {
       FilterKBase kbase = new FilterKBaseImpl(getAgentName(), null);
       new DefaultFilterKBaseLoader().load(kbase, myStandardCustomization);
       return kbase;
   } 
    ...
    * may be customized by overridding this method.
    * 
...
/**
   ...
   public void interpret(String formula) {
	   interpret(SL.fromFormula(formula));
   } 
