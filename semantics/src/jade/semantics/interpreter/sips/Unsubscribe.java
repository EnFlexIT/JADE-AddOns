...
//    
    ...
    public Unsubscribe(SemanticCapabilities capabilities, String observedProperty, String goal) {
        ...
        String g = "??goal";
        if (goal != null) g = goal;
        subscribeObservedPattern = SL.fromFormula("(B "+ myCapabilities.getAgentName() + " " + property +")");
        subscribeEventPattern = SL.fromFormula("(B "+ myCapabilities.getAgentName() +" (I ??subscriber " + g + "))");
//        unsubscribePattern1 = 
//            SL.fromFormula("(B "+ myCapabilities.getAgentName() +" (or " +
//                    "     (not (B " + myCapabilities.getAgentName() + " " + property + " )) " +
//                    "     (or (not (I ??subscriber ??goal))" +
//                    "(forall ??e (not (done ??e (not (B " + myCapabilities.getAgentName() + " " + property + " ))))))))");
        unsubscribePattern2 = 
            SL.fromFormula("(B "+ myCapabilities.getAgentName() +" (not (I ??subscriber " + g + ")))");
        universalPattern = 
            SL.fromFormula("??phi");
