...
public abstract class BeliefTransferSIPAdapter extends
	...
	public BeliefTransferSIPAdapter(SemanticCapabilities capabilities,
			...
			boolean notPattern, boolean allPattern) {
		this(capabilities,
				SL.fromFormula(formulaToBelievePattern),
				(originatingAgentPattern==null ?
						null :
						SL.fromTerm(originatingAgentPattern)),
                ...
                if (belief != null) {
    				...
    				if (matchAgent != null) {
    					...
    					if ( matchFormula != null) {
    						...
    						return doApply(matchFormula, matchAgent, acceptResult, new ArrayList(), sr);
    					}
					}
                }
            } 
	...
	/**
	 * Method to be overriden in each sub-class, to control the belief adoption
