...
public class SemanticInterpreterBehaviour extends CyclicBehaviour {
     ...
     */
    static private Formula donePattern = SL.fromFormula("(B ??agent (done ??act))");
     ...
     */
    private Formula NotUnderstandableDefaultPattern = SL.fromFormula("(B myself (not-understandable not-understandable null))");
    ...
    public SemanticInterpreterBehaviour(MessageTemplate msgTemplate, SemanticCapabilities capabilities) {
        ...
        NotUnderstandablePattern = SL.fromFormula("(B ??agent (not-understandable ??reason ??object))");
        NotUnderstoodContentPattern = SL.fromContent("(??act (??reason ??object))");
    ...
    private SemanticRepresentation directSRFromACLMessage(ACLMessage msg) {
        try {
        	...
        	if ( action == null || action instanceof NotUnderstood ) {
                return new SemanticRepresentation(new TrueNode()); // FIXME
        	}
        	else {
    ...
    protected boolean checkNotUnderstandable(ArrayList list) {
        for(int i = 0; i < list.size(); i++) {
            ...
            if( sr.getSLRepresentation() instanceof FalseNode || 
                ...
                ActionExpression msg = new ActionExpressionNode(SL.fromTerm(sr.getMessage().getSender().toString()),
                        SL.fromTerm(sr.getMessage().toString()));
