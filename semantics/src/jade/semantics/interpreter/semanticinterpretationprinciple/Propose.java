 ...
 * @version 
 */
public class Propose extends SemanticInterpretationPrincipleImpl {
     ...
     */
    public Propose(SemanticAgent agent) {
        setAgent(agent);
        pattern = SLPatternManip.fromFormula("(or (not (I " + agent.getAgentName() + " (done (action ??agent ??act) ??condition))) (I ??agent (done (action ??agent ??act) ??condition)))");
    ...
    public ArrayList apply(SemanticRepresentation sr)
        ...
        try {
            ...
            if (applyResult != null  
                    ...
                    agent.getMyStandardCustomization().trapProposal(applyResult.getTerm("??agent"), 
