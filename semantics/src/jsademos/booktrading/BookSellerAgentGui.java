...
public class BookSellerAgentGui  extends JFrame implements IBookSellerAgentGui {
	...
	static final IdentifyingExpression BOOKS_IRE = (IdentifyingExpression)
		SL.fromTerm("(some (sequence ?i ?t ?p ?m ?d) " +
	...
	class BookSellerTableModel implements TableModel {
		...
		public Object getValueAt(int rowIndex,int columnIndex) {
			...
			if ( columnIndex == 4 ) {
				...
				if ( sec > 60 ) {
					return new WordConstantNode(new Long((sec/60)+1).toString()+" min");
				}
				else {
					return new WordConstantNode(new Long(sec).toString()+" sec");
				}
			}
			else if ( columnIndex == 2 || columnIndex == 3 ) {
				return new WordConstantNode(c.realValue().toString()+" ¤");
			}
			else if ( columnIndex == 0 || columnIndex == 1 ) {
				return new WordConstantNode(c.stringValue());
			}
						...
						myCapabilities.getAgent().addBehaviour(
								new OneShotBehaviour(myCapabilities.getAgent()) {
									public void action() {
										...
										.instantiate("isbn", new StringConstantNode(isbnField.getText()))
										.instantiate("price", new RealConstantNode(new Double(priceField.getText())))
										.instantiate("min_price", new RealConstantNode(new Double(minPriceField.getText())))
										.instantiate("delay", new IntegerConstantNode(new Long(delayField.getText())))
