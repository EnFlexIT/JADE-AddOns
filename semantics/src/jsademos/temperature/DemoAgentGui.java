       ...
       if ( showkbase && agent instanceof SemanticAgent ) {
           ...
           mainPanel.addChangeListener(new ChangeListener() {
               public void stateChanged(ChangeEvent e) {
                   kbaseList.setListData(((SemanticAgent)agent).getSemanticCapabilities().getMyKBase().toStrings());
               }
           });
       }
       else {
           ...
           p.add(new JButton(new AbstractAction("Assert") {
               public void actionPerformed(ActionEvent evt) {
                   ...
                   if (text.getText() != null && !text.getText().trim().equals("")) {
                       result = SL.fromFormula(text.getText());
                   } else {
                       result = null;
                   }
                   if (result != null) {
               ...
               public void actionPerformed(ActionEvent evt) {
                   if ( kbaseList.getSelectedIndex() != -1 ) {
                       try {
						   ...
						   ((SemanticAgent)agent).getSemanticCapabilities().getMyKBase().retractFormula((Formula)formPattern);
                           kbaseList.setListData(((SemanticAgent)agent).getSemanticCapabilities().getMyKBase().toStrings());
                       }
