...
public class DisplayAgent extends SemanticAgent {
  ...
  int precision = Precision.UNSPECIFIED;
   ...
   = (ActionExpression)SL.fromTerm("(action ??receiver (INFORM-REF :content \"((any ?x (temperature ?x)))\" :receiver (set ??agent) :sender ??receiver))");
   static final IdentifyingExpression CFP_IRE 
   = (IdentifyingExpression)SL.fromTerm("(any ?x (precision ?x))");
   static final Formula CFP_CONDITION 
   = SL.fromFormula("(precision ??X)");
   static final IdentifyingExpression SUBSCRIBE_IRE 
   = (IdentifyingExpression)SL.fromTerm("(iota ?x (temperature ?x))");
   static final IdentifyingExpression SUBSCRIBE_DF_IRE 
   = (IdentifyingExpression)SL.fromTerm("(all ?x (sensor ?x))");
