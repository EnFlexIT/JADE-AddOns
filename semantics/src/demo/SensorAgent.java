 ...
 */
public class SensorAgent extends SemanticAgentBase {
    /**
     * Panel of the sensor
     */
    ...
    /** ****************************************************************** */
    /**
     * Sets the color of the sensor (yellow or gray) depending of the choice
     * of the display 
    ...
    public void setup() {
        super.setup();
        getSemanticCapabilities().getSemanticInterpreterBehaviour().interpret("(B "+getSemanticCapabilities().getAgentName()+"(precision "+Integer.parseInt((String)getArguments()[0])+"))");
        guiSetup();
        setSubscribed(false);
        dfagent = Tools.AID2Term(new AID(getArguments()[1].toString(),true));
        onFormula = SLPatternManip.fromFormula("(sensor "+getSemanticCapabilities().getAgentName()+")");
        offFormula = SLPatternManip.fromFormula("(not (sensor "+getSemanticCapabilities().getAgentName()+"))");
    } // End of setup
    ...
    protected void guiSetup() {
        ...
        final JSlider slider = new JSlider(JSlider.VERTICAL, -20*tenPowerPrecision, 50*tenPowerPrecision, 0);
        ...
        toggleButton.addActionListener(new AbstractAction("On") {
            public void actionPerformed(ActionEvent evt) {
                if (toggleButton.isSelected()) {
                    ...
                    getSemanticCapabilities().sendCommunicativeAction(getSemanticCapabilities().createInform(onFormula, dfagent));
                }
                else {
                    //Informs the df agent that is "off"
                    toggleButton.setIcon(DemoAgentGui.getIcon("red"));
                    toggleButton.setText("Off");
                    getSemanticCapabilities().sendCommunicativeAction(getSemanticCapabilities().createInform(offFormula, dfagent));					
                }}});
