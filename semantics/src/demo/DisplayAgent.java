...
public class DisplayAgent extends SemanticAgentBase {

    /**
     * Different precisions
     */
    enum Precision {UNSPECIFIED, LEAST, MOST};
    /**
     ...
     * Selected precision
     */
    Constant selectedPrecision = new IntegerConstantNode(0L);
    ...
    = (ActionExpression)SLPatternManip.fromTerm("(action ??receiver (INFORM-REF :content \"((any ?x (temperature ?x)))\" :receiver (set ??agent) :sender ??receiver))");
    static final IdentifyingExpression CFP_IRE 
    = (IdentifyingExpression)SLPatternManip.fromTerm("(any ?x (precision ?x))");
    ...
    protected void handleProposal(Constant precision, Term agent, ActionExpression action, Formula condition) {
        if ( selectedAgent == null ) {
            ...
            if (  ( this.precision == Precision.MOST && precision.intValue() > selectedPrecision.intValue() )
                    || ( this.precision == Precision.LEAST && precision.intValue() < selectedPrecision.intValue() )) {
                ((ActionExpressionNode)action).as_agent(selectedAgent);
                // Rejects the proposition
                getSemanticCapabilities().sendCommunicativeAction(getSemanticCapabilities().createRejectProposal(action, 
                        selectedCondition,
                        new TrueNode(),
                        selectedAgent));
                selectedAgent = agent;
     ...
     * @param precision the precision
     */
    protected void setPrecision(Precision precision) {
        display.precisionPanel.setStatus(false);
        this.precision = precision;
        try {
            // Unsubscribe to the current selectedAgent if necessary
            if ( selectedAgent != null ) {
                getSemanticCapabilities().sendCommunicativeAction(getSemanticCapabilities().createUnsubscribe(SUBSCRIBE_IRE, selectedAgent));
                selectedAgent = null;
            }
            // Send a query-ref to the dfagent
            getSemanticCapabilities().sendCommunicativeAction(getSemanticCapabilities().createQueryRef(SUBSCRIBE_DF_IRE, dfagent));
            ...
            addBehaviour(new WakerBehaviour(this, 2000) {
                protected void handleElapsedTimeout() {
                    if ( selectedAgent != null ) {
                        getSemanticCapabilities().sendCommunicativeAction(getSemanticCapabilities().createSubscribe(SUBSCRIBE_IRE, selectedAgent));
    ...
    class TemperatureDisplay extends JFrame {
        ...
        void setTemperature(Double temp) {
            ...
            long precision = selectedPrecision.intValue();
            if ( precision == 0 ) {
                setSize(450,270);
            }
            else {
    ...
    class PrecisionChoicePanel extends JPanel {
        ...
        {
            if ( ok ) {
                status.setText("Subscribed to "+Tools.term2AID(selectedAgent).getName());
            }
            else {
