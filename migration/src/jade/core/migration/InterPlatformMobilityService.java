...
public class InterPlatformMobilityService extends BaseService {
	...
	public void init(AgentContainer ac, Profile p) throws ProfileException {
		...
		try {
			...
			if (p.getParameter(AgentManagementService.AGENTS_PATH, "").equals(
				...
				p.setParameter(AgentManagementService.AGENTS_PATH, System
	...
	private class CommandSourceSink implements Sink {
		...
		private void launchAgent(VerticalCommand cmd) {
			...
			try {
				//Register jar in CodeLocator and deserialize agent instance
				String jarfile = getLocator().registerAgent(name, jar);
				File file = new File(jarfile);
				JarClassLoader loader = new JarClassLoader(file, getClass()
						.getClassLoader());
				Deserializer des = new Deserializer(new ByteArrayInputStream(
						instance), loader);
		...
		private void handleDeleteAgentReferences(VerticalCommand cmd) {
			...
			try {
				...
				if (location != null) {
					...
					//In case of agent do has his code here, we obtain from mobility service
		...
		private byte[] handleGetAgentCode(AID name, String className) {
			...
			try {
				...
				if (location != null) {
					...
					// create jar agent
					createJar(name, Class.forName(className));

					// load and return the new jar
					location = getLocator().getAgentCodeLocation(name);
					if (location != null) {
						File jar = new File(location);
						rawJar = getJarByteArray(jar);
						if (logger.isLoggable(Logger.FINE))
	...
	private void createJar(AID aid, Class c) {
		...
		try {
			...
			//Register JAR agent.
			getLocator().registerAgent(aid, cal.createJar(true));
	...
	private class Deserializer extends ObjectInputStream {
		...
		protected Class resolveClass(ObjectStreamClass v) throws IOException,
			...
			return _loader.loadClass(v.getName());
		}
