...
public class TestMain2MainAndClone extends Test {
  ...
  public Behaviour load(Agent a) throws TestException {
    ...
    AID resp = TestUtility.createAgent(a, "pep", "test.migration.agents.ClonningAgent",args, remoteAMS, null);
  ...
  private class ResponderBehaviour extends SimpleBehaviour {
     ...
     */
    public void action() {
      AID clone = new AID("myClone@Remote-platform-2",AID.ISGUID);
      if(_block) _msg = myAgent.receive(
                            MessageTemplate.or(
                                MessageTemplate.MatchSender(_movAgent), 
                                MessageTemplate.MatchSender(clone)));
      if(_msg != null || !_block){
        if(_block){
