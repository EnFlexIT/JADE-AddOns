 ...
 * 
 */
public class TestMain2Cont2Main2RMain extends Test {
  ...
  public Behaviour load(Agent a) throws TestException {
    ...
    args[1] = "DestinationContainer3";
    args[2] = "Main-Container";
    args[3] = remoteAMS2.getName() + "#" + remoteAMS2.getAddressesArray()[0];
    
    
    int port = Integer.parseInt((String)getGroupArgument(MigrationTesterAgent.REMOTE_PLATFORM_PORT_KEY));
    container = TestUtility.launchJadeInstance("DestinationContainer3", 
                                    null, 
                                   "-container -container-name DestinationContainer3 -host localhost -port "
                                   + port + " " 
    ...
    AID resp = TestUtility.createAgent(a, "pep", "test.migration.agents.GenericMovingAgent",args, remoteAMS, "Main-Container");
  ...
  private class ResponderBehaviour extends SimpleBehaviour {
    ...
    public void action() {
      ...
      if(_msg != null || !_block){
        if(_block){
         block(5000); //block 10 seconds waiting finalisation of migration process
