...
public class ActivePlatformService extends MulticastBehaviour implements
	...
	private RecordPlatform extractRecord(ACLMessage msg) {
		RecordPlatform record = null;
		if (FactoryAgent.JAVASERIALIZATION_LANGUAGE.equals(msg
				.getLanguage())) {
			try {
		...
		} else if (FactoryAgent.STRING_LANGUAGE.equals(msg
				.getLanguage())) {
	...
	public void sendActivePlatform(ACLMessage reply) {
		...
		reply.setLanguage(FactoryAgent.JAVASERIALIZATION_LANGUAGE);
		try {
			synchronized (activePlatform) {
			...
			reply.setLanguage(FactoryAgent.STRING_LANGUAGE);
	...
	public void update(Observable o, Object arg) {
		if (arg instanceof MulticastMessage) {
			...
			aclmsg.setLanguage(FactoryAgent.JAVASERIALIZATION_LANGUAGE);
