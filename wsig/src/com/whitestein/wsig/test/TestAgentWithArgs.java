 ...
 */
public class TestAgentWithArgs extends Agent {

	private static String nickName = "TestAgentWithArgs default name";
	private int convId = 0;
	private Logger log = Logger.getLogger(TestAgentForFindPlace.class.getName());
	public String wsdlOperation = "getVersion";  // default
	public String opArgs = ""; // default none arguments
	...
	private void parseArguments() {
		...
		if ( args[0] instanceof String ) {
			wsdlOperation = (String) args[0];
		} else {
			wsdlOperation = args[0].toString();
		}
		log.info("An operation: " + wsdlOperation );
		if ( args.length < 2 ) {
			return;
		}

		// get an operation's arguments in SL0 format
		if ( args[1] instanceof String ) {
			opArgs = (String) args[1];
		} else {
			opArgs = args[1].toString();
		}
		log.info("An arguments: " + opArgs );
	}
