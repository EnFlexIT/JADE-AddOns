...
public class EncryptionFilter {
	...
	public void init(AgentContainer myContainer ) {
		...
		try {
			...
			System.out.println("--- Security service is "+ss);
		...
		public boolean accept(VerticalCommand cmd) {
			...
			if (name.equals(MessagingSlice.SEND_MESSAGE)) {
				...
				try {
					...
					System.out.println("Encryption filter: Processing message from "+msg.getACLMessage().getSender().getName()+": Envelope is "+env);
					if ((env != null)&&((so=SecurityService.getSecurityObject(env,SecurityObject.ENCRYPT)) != null)) {
						...
						if (acl != null) { 
							acl.setContent(null);
							System.out.println("Message content set to null !!!!");
							so.setConversationId(acl.getConversationId());
						}
						ss.encode(so);
						System.out.println("--- Security Object encoded");
						// update the Generic Message in the command
						msg.update(acl, env, enc);
						System.out.println("--- GenericMessage updated");
						System.out.println(msg.getACLMessage());
