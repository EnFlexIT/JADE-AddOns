...
public class OntologyBeanGeneratorTab
  ...
  private String getSuperClass(Cls theCls) {
    ...
    String superClassName = converter.toJavaTypeName(superClass);

    //        String superClassName = superClass.getName();
    //        superClassName = superClassName.replace(' ', '_');
    //        superClassName = superClassName.replace('-', '_');
    //        superClassName = OntologyBeanGeneratorUtil.firstUpper(superClassName);
    // Implementing one of the JADE primitives
    if (superClassName.equals(CONCEPT) || superClassName.equals(AGENTACTION) ||
        superClassName.equals(PREDICATE) ||
        superClassName.equals(OWLCONCEPT) || superClassName.equals(OWLAGENTACTION) ||
        superClassName.equals(OWLPREDICATE)
    ) {
      // Java specific inheritance
      String javaBaseClass = getJavaSuperClass(theCls);
      if (javaBaseClass != null) {
        result.append(" extends ");
        javaBaseClass = javaBaseClass.replaceAll("jade\\x58","");
  ...
  private boolean isJADEPrimitive(Cls theCls) {
    ...
    if (className.equalsIgnoreCase(AID) || className.equalsIgnoreCase(CONCEPT) ||
        ...
        className.equalsIgnoreCase(OWLAID) || className.equalsIgnoreCase(OWLCONCEPT) ||
        className.equalsIgnoreCase(OWLAGENTACTION) ||
        className.equalsIgnoreCase(OWLPREDICATE) 
  ...
  private void addClass(Cls theCls, String collectionName) {
    ontologyHeader.insert(0,
                          ...
                          theCls.getName() + "\";\n");
    if (collectionName.equals(AID) || collectionName.equals(OWLAID)) {
      collectionName = "Concept";
    }
    collectionName = collectionName.replaceAll("jade\\x3a","");
  ...
  private void addRole(Cls theCls, Slot theSlot) {
    try {
       ...
       */
      System.out.println("Warning: Something went wrong with Slot: " +
                         theSlot.getName() + " of class: " + theCls.getName());
...
//      ontologyType = "BasicOntology.STRING";
    ...
    if (c.size() == 1) {
      ...
      if (ontologyType.equalsIgnoreCase(CONCEPT) ||
          ...
          ontologyType.equalsIgnoreCase(OWLCONCEPT) ||
          ontologyType.equalsIgnoreCase(OWLAID) ||
          ontologyType.equalsIgnoreCase(OWLAGENTACTION) ||
          ontologyType.equalsIgnoreCase(OWLPREDICATE)) {
        if (ontologyType.equalsIgnoreCase(AID)||ontologyType.equalsIgnoreCase(OWLAID)) {
    ...
    ontologyType = ontologyType.replaceAll("jade\\x3a","");
...
//    }
    ...
    ontologyBody.append(".add(" + roleName + ", " + termSchemaString + ", " +
                        roleParams + ");");
    ontologyBody.append("\n");
...
//        "    super(ONTOLOGY_NAME, BasicOntology.getInstance(), introspect);\n");
  ...
  private void createSlot(PrintStream ps, Cls theCls, Slot theSlot,
    ...
    if (!theSlot.getOwnSlotAllowsMultipleValues(theSlot)) {
      ...
      String propertyStr = "";
      if (fullBeanSupport) {
        oldStr = "     List oldList = new ArrayList();\n";
        oldStr += "     ProtegeTools.copyCollection(this." + theVarName +
            ",oldList);\n";
  ...
  void doIt() {
    ...
    //cheking type of knowledge base
    Cls owlThing = itsKB.getCls(OWLTHING);
    if (owlThing != null) {
      owlFile = true;
    }
    ...
    Collection conceptCol;
    Collection aidCol;
    Collection agtactClsCol;
    Collection predCol;
    
    //if owl
if (owlFile) {
	conceptCls= itsKB.getCls(OWLCONCEPT);
	AIDCls=itsKB.getCls(OWLAID);
	agtactCls=itsKB.getCls(OWLAGENTACTION);
	predClS=itsKB.getCls(OWLPREDICATE);    
    
    
    conceptCol = new HashSet(conceptCls.getSubclasses());
    aidCol = new HashSet(AIDCls.getSubclasses());
    agtactClsCol = new HashSet(agtactCls.getSubclasses());
    predCol = new HashSet(predClS.getSubclasses());
//  filter the classes.  i.e. remove duplicates, system and ignored classes
    filterOWLClasses(conceptCol, aidCol, agtactClsCol, predCol);

} else {
	conceptCls=itsKB.getCls(CONCEPT);
	AIDCls= itsKB.getCls(AID);
	agtactCls=itsKB.getCls(AGENTACTION);
	predClS= itsKB.getCls(PREDICATE);    
...
//  filter the classes.  i.e. remove duplicates, system and ignored classes
    ...
    if (owlFile) {
        ...
        generateByCollection(conceptCol, OWLCONCEPT);
        generateByCollection(agtactClsCol, OWLAGENTACTION);
        generateByCollection(aidCol, OWLAID);
        generateByCollection(predCol, OWLPREDICATE);
    }    	
    else {
    
    // Generate each type of entity, Concepts, Actions, AIDs, and Predicates
    generateByCollection(conceptCol, CONCEPT);
    generateByCollection(agtactClsCol, AGENTACTION);
    generateByCollection(aidCol, AID);
    generateByCollection(predCol, PREDICATE);
    }
    ...
    // create help classes
    String dir = converter.toDir(packageName, dirName);
    OntologyBeanGeneratorUtil.createHelpClasses(dir, packageName);
   ...
   *@param  predCol       Description of the Parameter
   */
  private void filterOWLClasses(Collection conceptCol, Collection aidCol,
                             Collection agtactClsCol, Collection predCol) {
    // ensure we don't regenerate the classes Concept, AID, AgentAction
    // as they also inherit from Concept
      
	  if (itsKB.getCls(OWLCONCEPT) == null) {
      System.out.println("no OWLConcepts found");
    }
    else {
      //   System.out.println("removing: " + itsKB.getCls(CONCEPT).getClass());
      conceptCol.remove(itsKB.getCls(OWLCONCEPT));
    }
    
    
    
    if (itsKB.getCls(OWLAID) != null) {
      conceptCol.remove(itsKB.getCls(OWLAID));
    }
    if (itsKB.getCls(OWLAGENTACTION) != null) {
      conceptCol.remove(itsKB.getCls(OWLAGENTACTION));
  ...
  private void finishUpRole(Cls theCls) {
    ...
    // thanks to J.Picault
    if (!microSupport) {
      ontologyHeader.append("    add(" +
                            converter.toSchemaVariableName(theCls, null) +
                            ", " + this.packageName + "." +
      ...
      ontologyHeader.append("    add(" +
                            converter.toSchemaVariableName(theCls, null) +
                            ", Class.forName(\"" + this.packageName + "." +
                            converter.toJavaTypeName(theCls) + "\"));\n");
    }
    ...
    while (classIter.hasNext()) {
      ...
      if (theCls.getName().charAt(0) != ':') {
        ...
        try {
          ...
          if (owlFile) {
            theSlots = new HashSet(OntologyBeanGeneratorUtil.
                                   getClassProperties(
                theCls));
...
//    private Jalopy jalopy;
  ...
  public void initialize() {
    ...
    ImageIcon beanIcon = new ImageIcon(this.getClass()
                                       .getResource("graphics/jadeicon.gif"));
    this.setIcon(beanIcon);
    setLabel("Ontology Bean Generator");
    this.setToolTipText("Ontology Bean Generator");
    ...
    Cls owlThing = itsKB.getCls(OWLTHING);
    if (owlThing != null) {
      System.out.println("Ontology Bean Generator: recognize" + itsKB.getName() +
                         " as an owl database.");
      owlFile = true;
      return;
    }
    Cls projectAnno = itsKB.getCls(PROJECTANNOTATION);
    if (projectAnno == null && !owlFile) {
      System.out.println(
...
//       JOptionPane.ERROR_MESSAGE);
    ...
    if (projectAnno.getInstanceCount() == 0) {
      ...
      this.projectAnnoInstance = (Instance) projectAnno.getInstances().
          iterator().
          next();
  ...
  private void reset() {
    ...
    converter = new NameConverter(itsKB, useJADENames, owlFile);
...
// Constants for Protege
  private final String JADE_CLASS = ":JADE-CLASS";
  private final String JADE_IGNORED_SLOT = ":JADE-IGNORE";
  private final String JADE_JAVA_BASE_CLASS_SLOT = ":JADE-JAVA-BASE-CLASS";
  private final String JADE_JAVA_CODE_SLOT = ":JADE-JAVA-CODE";
  private final String JADE_NAME_SLOT = ":JADE-NAME";
  private final String JADE_SLOT = ":JADE-SLOT";
  private final String JADE_UNNAMED_SLOT = ":JADE-UNNAMED";
  private final String PROTEGE_DOC = ":DOCUMENTATION";
  private final static String AGENTACTION = "AgentAction";
  private final static String OWLAGENTACTION = "jade:AgentAction";
  private final static String AID = "AID";
  private final static String OWLAID = "jade:AID";
  private final static String CONCEPT = "Concept";
  private final static String OWLCONCEPT = "jade:Concept";
  private final static String OWLTHING = "owl:Thing";
  private final static String LOCATION_SLOT = ":LOCATION";
  private final static String ONTOLOGYNAME_SLOT = ":ONTOLOGYNAME";
  private final static String PACKAGE_SLOT = ":PACKAGE";
  private final static String PREDICATE = "Predicate";
  private final static String OWLPREDICATE = "jade:Predicate";
  private final static String PROJECTANNOTATION = ":PROJECT-ANNOTATION";
  private final static String SUPPORT_SLOT = ":SUPPORT";
  /*
