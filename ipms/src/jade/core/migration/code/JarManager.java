...
package jade.core.migration.code;
...
public class JarManager {
	...
	public JarManager(String agentsPath) {
	...
	public synchronized String registerAgent(AID name, File source) throws Exception {
		File destination = null;
		
		do {
			destination = new File(_jarFolder + File.separator + randomString(8) + ".jar");
		} while (destination.exists());
		
		// Copy original Jar file to the JarFolder.
		try {
		...
		return registerAgentJar(name, destination, false);
	...
	private synchronized String registerAgentJar(AID name, File f, boolean userCreatedJar) throws Exception {
	...
	private synchronized String registerJar(AID name, String path, String jarUID, boolean userCreatedJar) {
...
//			throw new IOException();
	...
	public synchronized boolean cloneAgent(AID oldName, AID newName) {
		
		if (_agentsUsingJar.containsKey(oldName)) {
			
			// Get Jar Code Identifier.
			String jarCID = (String) _agentsUsingJar.get(oldName);
			
			// Increment reference to this code.
			Row r = (Row) _jarHash.get(jarCID);
			r.incRef();
			
			// Create a new entry for the new agent.
			_agentsUsingJar.put(newName, jarCID);
