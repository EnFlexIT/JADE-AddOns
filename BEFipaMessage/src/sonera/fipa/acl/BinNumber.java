...
public class BinNumber extends BinRep {
	...
	public static void main (String[] args)
	{
		...
		ByteArray bArray = bn.toBin ("11");

		ByteArray byteArray = new ByteArray ();
		byteArray.add ((byte) 0x22);
		byteArray.add ((byte) 0x00);

		System.out.println ("ENCODED \"11\": " + bArray);
		System.out.println ();
		ByteArray bArray2 = bn.fromBin (byteArray);		
		System.out.println ("DECODED: " + bArray2);
		System.out.println ();
		char[] chars = new char[bArray2.length ()];
		byte[] bytes = bArray2.get ();
		for (int i = 0; i < chars.length; i++)
		{
			chars[i] = (char) bytes[i];
		}
		System.out.println ("DECODED AS STRING: " + new String (chars));
	}
}
